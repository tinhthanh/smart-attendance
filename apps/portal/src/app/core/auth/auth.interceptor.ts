...
export const authInterceptor: HttpInterceptorFn = (
  ...
  if (token && !shouldSkip(req.url)) {
    req = addToken(req, token) as HttpRequest<unknown>;
  }
  ...
  return next(req).pipe(
    catchError((err: HttpErrorResponse) => {
          ...
          auth
            ...
            .then((ok) => {
              ...
              if (ok) {
                ...
                next(addToken(req, newToken) as HttpRequest<unknown>).subscribe(
                  subscriber
                );
              } else {
                refreshSubject.next(null);
      ...
      return refreshSubject.pipe(
        ...
        take(1),
        switchMap((newToken) =>
          next(addToken(req, newToken!) as HttpRequest<unknown>)
        )
      );
    })
  );
